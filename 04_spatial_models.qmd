...
In 2023 New Zealand scrapped an anti-smoking law that would mean a generational smoking bad for anyone born after 2008. So, here we will explore the factors that might affect smoking in New Zealan and see if they vary spatially.
...
The figure below explains the geographies of New Zealand, we could replcaite this analysis for SA1 or SA2 data.

[![NZ geographies](prac4_images/Figure-1_GeoHierarchy_Diagram-with-legend-changes.jpg){fig-align="center" width="400"}](https://www.stats.govt.nz/methods/geographic-hierarchy#:~:text=Moving%20up%20the%20hierarchy%2C%20statistical,and%20territorial%20authorities%20and%20represent)
...
districts <- st_read("prac4_data/statsnzterritorial-authority-2018-generalised-SHP/territorial-authority-2018-generalised.shp")
...
* median income
* home ownership
* no qualification, [highschool certificate](https://www.careers.govt.nz/articles/ncea-compared-with-school-c-and-bursary/#cID_8307)
...
districts_model_data<- districts%>%
  clean_names(.)%>%
...
1. Chatham Islands has been included and is also a large positive residual.
...
districts_model3_data<- districts%>%
  clean_names(.)%>%
  left_join(., model3_residuals,
            by=c("ta2018_v1"="area_code"))

tm_shape(districts_model3_data) +
  tm_polygons(".resid",
              palette = "RdYlBu")
...
districts_model3_data <- districts_model3_data %>%
  filter(ta2018_v1 != "999" & ta2018_v1 != "067")

coordsW <- districts_model3_data%>%
  st_centroid()%>%
  st_geometry()
...
LWard_nb <- districts_model3_data %>%
  poly2nb(., queen=T)
...
```{r, message=FALSE, cache=FALSE}
Nearest_neighbour <- districts_model3_data %>%
  st_drop_geometry()%>%
  dplyr::select(.resid)%>%
  pull()%>%
...
districts_model3_data_GWR <- cbind(districts_model3_data,coordsW2)

GWRbandwidth <- gwr.sel(percent_regular_smoker ~
               percent_no_qualification + 
               percent_home_owner +
               census_2018_total_personal_income_median_curp_15years_and_over,
               data = districts_model3_data_GWR,  
                        coords=cbind(districts_model3_data_GWR$X, districts_model3_data_GWR$Y),
                  adapt=T)
GWRbandwidth
```
...
districts_model3_data_GWR_results <- districts_model3_data_GWR %>%
...
```{r}
tm_shape(districts_model3_data_GWR_results) +
...
districts_model3_data_GWR_results <- districts_model3_data_GWR_results %>%
